/* ============================================================
   ANIMA HONEY — script.js
   ES Module. All logic runs after top-level awaits resolve.
   ============================================================ */
...
try {
    ...
    if (!configValid) throw new Error("Firebase config is empty.");
    ...
    window.subscribeNewsletter = async () => {
        ...
        try {
            ...
            await setDoc(docRef, { email: rawEmail, timestamp: serverTimestamp() });
...
function toggleAuthModal(show) {
    if (show && !auth) { showToast("Login unavailable right now.", "info"); return; }
            ...
            <div class="bg-white p-6 rounded-3xl shadow-sm hover:shadow-xl transition-all border border-gray-50 h-full flex flex-col items-center">
                ...
                <div class="flex gap-3 mt-4 w-full">
                    <button onclick="addToCart(${p.id})"
                            ...
                            aria-label="Add ${p.name} to cart">Pre Order</button>
                    <button onclick="smoothScrollTo('#blog')"
                            ...
                            aria-label="View ${p.name} benefits">Benefits</button>
                </div>
            </div>
        </div>
    `).join('');
...
window.addToCart = (productId) => {
    ...
    if (item) { item.qty++; }
    else { window.cart.push({ cartId, id: productId, name: product.name, weight, price: variant.price, img: product.img, qty: 1 }); }
    updateCart();
        ...
        container.innerHTML = window.cart.map(item => `
            <div class="flex items-center gap-4 border-b pb-4">
                ...
                <div class="flex flex-col items-end gap-2 flex-shrink-0">
                    ...
                    <button onclick="removeOneFromCart('${item.cartId}')" class="text-red-500 p-1 hover:text-red-700" aria-label="Remove from cart">
...
function toggleQRModal(show) {
    document.getElementById('qrModal')?.classList.toggle('hidden', !show);
}
...
function toggleFssaiModal(show) {
    const m = document.getElementById('fssaiModal');
    if (!m) return;
    m.classList.toggle('hidden', !show);
    document.body.style.overflow = show ? 'hidden' : '';
}
window.toggleFssaiModal = toggleFssaiModal;

/* ============================================================
   LEGAL CONTENT
   ============================================================ */
window.openPrivacy = () => toggleLegalModal(true, 'Privacy Policy', `
...
window.openTerms = () => toggleLegalModal(true, 'Terms & Conditions', `
...
window.openRefund = () => toggleLegalModal(true, 'Shipping & Returns', `
...
window.slideGallery = (dir) => {
    ...
    document.getElementById("gallery-grid").style.transform =
        `translateX(-${window.currentGalleryIdx * (itemWidth + 32)}px)`;
};
...
function initHamburger() {
    ...
    // Build the mobile dropdown menu
    ...
    links.forEach(({ href, label }) => {
        ...
        a.href         = href;
        a.textContent  = label;
        a.style.cssText = 'display:block;padding:10px 0;color:#1f2937;font-weight:500;font-size:15px;border-bottom:1px solid #fde68a;';
    ...
    // Insert menu right after the nav
    const nav = hamburgerBtn.closest('nav');
    ...
    // Expose so Esc key can also close it
...
function initKeyboard() {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            ...
            const fssaiModal = document.getElementById('fssaiModal');
            const mobileMenu = document.getElementById('mobileMenu');

            if (cartPanel?.classList.contains('open'))         { toggleCart(false);         return; }
            if (!authModal?.classList.contains('hidden'))      { toggleAuthModal(false);    return; }
            if (!qrModal?.classList.contains('hidden'))        { toggleQRModal(false);      return; }
            if (!legalModal?.classList.contains('hidden'))     { toggleLegalModal(false);   return; }
            if (!fssaiModal?.classList.contains('hidden'))     { toggleFssaiModal(false);   return; }
            if (mobileMenu?.style.display === 'flex')         { window.closeMobileMenu?.(); }
        }
    });

    // ENTER — submit auth form from email or password field
    document.addEventListener('keydown', (e) => {
        ...
        if (active?.id === 'authEmail' || active?.id === 'authPass') {
            e.preventDefault();
...
/* ============================================================
   INIT — runs after all awaits above are settled
   ============================================================ */
function init() {
    ...
    // Smooth scroll for all plain anchor links in the page
...
// The module has already awaited Firebase above, so DOM is ready.
// If body is still loading, wait for it; otherwise run immediately.
